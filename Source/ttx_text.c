...
BOOL InitTextBuffer(struct TextBuffer *buffer, struct CleanupStack *stack) {
  ...
  buffer->lines = (struct TextLine *)allocVec(
      buffer->maxLines * sizeof(struct TextLine), MEMF_CLEAR);
  if (!buffer->lines) {
  ...
  buffer->lines[0].text =
      (STRPTR)allocVec(buffer->lines[0].allocated, MEMF_CLEAR);
...
BOOL LoadFile(STRPTR fileName, struct TextBuffer *buffer,
  ...
  while (FGets(fileHandle, lineBuffer, sizeof(lineBuffer) - 1) != NULL) {
    ...
    /* Expand line array if needed (capped at MAX_LINES) */
    if (i >= buffer->maxLines) {
      ...
      newLines = (struct TextLine *)allocVec(newMax * sizeof(struct TextLine),
                                             MEMF_CLEAR);
      if (!newLines) {
        FreeTextBuffer(buffer, stack);
        closeFile(fileHandle);
    ...
    buffer->lines[i].text =
        (STRPTR)allocVec(buffer->lines[i].allocated, MEMF_CLEAR);
    if (!buffer->lines[i].text) {
      FreeTextBuffer(buffer, stack);
      closeFile(fileHandle);
      return FALSE;
    ...
    if (i >= buffer->maxLines || i >= MAX_LINES) {
      break;
    }
...
BOOL CreateSuperBitMap(struct TextBuffer *buffer, struct Window *window) {
  ...
  /* Strategy 1: Try 1.5x window size with graphics board memory
   * (BMF_DISPLAYABLE) */
  ...
  while (multiplier >= 100) {
    ...
    /* Try allocation */
    buffer->superBitMap =
        AllocBitMap(tryWidth, tryHeight, depth, flags, displayBitMap);

    if (buffer->superBitMap) {
      /* Success! */
      buffer->superWidth = tryWidth;
      buffer->superHeight = tryHeight;
      Printf("[GFX] CreateSuperBitMap: SUCCESS (w=%lu, h=%lu, d=%lu, "
             "multiplier=%lu%%, flags=0x%08lx, bitmap=%lx)\n",
             buffer->superWidth, buffer->superHeight, depth, multiplier, flags,
             (ULONG)buffer->superBitMap);
      buffer->needsFullRedraw = TRUE;
      return TRUE;
    }
    ...
    if (flags & BMF_DISPLAYABLE) {
      ...
      if (buffer->superBitMap) {
        ...
        buffer->superHeight = tryHeight;
        Printf("[GFX] CreateSuperBitMap: SUCCESS (w=%lu, h=%lu, d=%lu, "
               "multiplier=%lu%%, chip RAM, bitmap=%lx)\n",
               buffer->superWidth, buffer->superHeight, depth, multiplier,
               (ULONG)buffer->superBitMap);
        buffer->needsFullRedraw = TRUE;
        return TRUE;
      }
  ...
  Printf("[GFX] CreateSuperBitMap: AllocBitMap failed for all sizes "
         "(window=%lux%lu, depth=%lu)\n",
...
/* Render text to window using ScrollLayer for optimized scrolling (Graphics
 * v39+) */
...
VOID MouseToCursor(struct TextBuffer *buffer, struct Window *window,
...
BOOL InsertText(struct TextBuffer *buffer, STRPTR text,
                struct CleanupStack *stack) {
  ULONG i = 0;
  UBYTE ch = 0;
...
BOOL SetCharAtCursor(struct TextBuffer *buffer, UBYTE ch,
                     struct CleanupStack *stack) {
  if (!buffer || buffer->cursorY >= buffer->lineCount || !stack) {
    return FALSE;
  }
...
BOOL ReplaceWordAtCursor(struct TextBuffer *buffer, STRPTR newWord,
                         struct CleanupStack *stack) {
  ULONG startX = 0;
  ULONG endX = 0;
  ULONG savedX = 0;
...
BOOL ConvertTabsToSpaces(struct TextBuffer *buffer,
                         struct CleanupStack *stack) {
  ULONG startY = 0;
  ULONG startX = 0;
  ULONG stopY = 0;
...
BOOL ConvertSpacesToTabs(struct TextBuffer *buffer,
                         struct CleanupStack *stack) {
